...
export default function Benefits() {
  ...
  const renderBenefit = ({ item }) => {
    ...
    if (item.type === "cash") {
      remaining =
        totalBudget - (item.records_count || 0) * (item.budget_amount || 0);
    } else {
      remaining =
        totalBudget - (item.records_count || 0) * (item.budget_quantity || 0);
    }
    ...
    return (
      ...
      >
        ...
        <Card.Content>
          ...
          <Text style={styles.detail}>
            👥 Locked Members: {item.locked_member_count || 0}
          </Text>
        </Card.Content>
      </Card>
    );
