...
export default function BenefitScanner() {
  const { benefitId } = useLocalSearchParams(); // get benefitId from route params
  ...
  const handleBarcodeScanned = async ({ data }) => {
    ...
    try {
      ...
      // 2. Save benefit record
      await api.post(`/benefit-records`, {
        member_id: member.member_profile.id, // ✅ member_profiles.id
        benefit_id: benefitId,
        status: "claimed",
        claimed_at: new Date().toISOString().slice(0, 19).replace("T", " "), // ✅ MySQL datetime
      });

      Alert.alert("✅ Success", "Benefit claimed successfully!");
    } catch (err) {
      console.error("Error:", err.response?.data || err.message);
      setError("Could not claim benefit.");
      setMemberData(null);
    }
  };
